...
const schemasDoc = YAML.load(path.join(__dirname, '../docs/swagger/schemas.yaml'));
const authDoc = YAML.load(path.join(__dirname, '../docs/swagger/auth.yaml'));
const childrenDoc = YAML.load(path.join(__dirname, '../docs/swagger/children.yaml'));
const parentsDoc = YAML.load(path.join(__dirname, '../docs/swagger/parents.yaml'));
const schoolsDoc = YAML.load(path.join(__dirname, '../docs/swagger/schools.yaml'));
const integrationsDoc = YAML.load(path.join(__dirname, '../docs/swagger/integrations.yaml'));
...
const options = {
  definition: {
    ...
    paths: paths,
    tags: [
      { name: 'Authentication', description: 'User authentication - register, login, OTP verification, password management' },
      { name: 'Children', description: 'Child management - create, read, update, delete child profiles with schedules' },
      { name: 'Parents', description: 'Parent profile and management' },
      { name: 'Admin - Parents', description: 'Admin parent management operations' },
      { name: 'Schools', description: 'School management with geospatial search capabilities' },
      { name: 'Integrations', description: 'Third-party service integrations (Google Maps, SMS, etc.)' }
    ]
  },
  apis: []
};
